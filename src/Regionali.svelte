<div class="ep-regionali">
   <div class="ep-regionali-wrapper" ref:wrapper>
         <div class="ep-regionali-box">
            <div class="flex-wrapper">
               <h3 class="ep-regionali-title" style="border-bottom: 3px solid #ddd;">{{title}}</h3>
               <div class="list-wrapper">
               {{#each regioni[0] as candidato, index}}
                  <div class="list">
         ...
         {{#if regioni[1]}}
            <div class="ep-regionali-box">
               <div class="flex-wrapper">
               <div class="flex-wrapper">
                  <h3 class="ep-regionali-title" style="border-bottom: 3px solid #ddd;">{{title2}}</h3>
                  <div class="list-wrapper">
                  {{#each regioni[1] as candidato, index}}
                     <div class="list">
                        <div class="img-wrapper" style="background-image:url({{candidato.picture}}); color:{{candidato.color}}"></div>
                        <div class="perc">{{candidato.perc}}{{#if candidato.perc2}}-{{candidato.perc2}}{{/if}}%</div>
                        <h4>{{candidato.cognome}} {{candidato.nome}}</h4>
...
export default {
   ...
   computed:{
      ...
      regioni: function(figures, figures2, configList, limit, imagePath){
         const tempArr = [figures, figures2];
         function returnRegione(regione){
            const out =  regione.filter((el, i) => i<limit).map(el =>{
         ...
         return [figures, figures2].map(datiRegione=>returnRegione(datiRegione));
      }
   }
}
</script>
